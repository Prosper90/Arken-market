...
const CreateMarket = () => {
  ...
  const addMultiOption = () => {
    ...
    setMultiOptions((prev) => {
      ...
      return [...prev, { id: Date.now(), label: "", prob: even }].map(
        (o, i) => ({
          ...o,
          prob: i === 0 ? even + rem : even,
        }),
      );
    });
  };
  ...
  const updateMultiLabel = (id, val) => {
    setMultiOptions((prev) =>
      prev.map((o) => (o.id === id ? { ...o, label: val } : o)),
    );
  };
  ...
  const runAI = async () => {
    ...
    const hasTimeframe =
      /\b(202[5-9]|203\d|january|february|march|april|may|june|july|august|september|october|november|december|q[1-4]|this week|this month|this year|by end|end of|before|after|within)\b/.test(
        q,
      );
    const hasVerifiableOutcome =
      /\b(price|market cap|volume|rate|percentage|usd|\$|billion|million|reach|hit|exceed|drop|fall|rise|win|lose|beat|pass|surpass|launch|list|delist|ban|approve|reject|elect|sign|fail)\b/.test(
        q,
      );
    ...
    const illegalTerms =
      /\b(kill|murder|attack|bomb|hack|steal|fraud|scam|illegal|weapon|drug|sex|nude|naked|terrorist|violence|abuse|exploit)\b/;
    checks.legal = !illegalTerms.test(q);

    const isPersonalTrivial =
      /\b(my (cat|dog|pet|friend|mom|dad|sister|brother|wife|husband|girlfriend|boyfriend)|eat today|sleep|wake up|go to|come home|call me|text me)\b/.test(
        q,
      );
    const isTooGeneric =
      /^will (it|this|that|he|she|they|we|i) /.test(q) && words.length < 7;
    const hasCryptoOrFinance =
      /\b(bitcoin|btc|eth|ethereum|sol|solana|crypto|token|coin|market|stock|price|fed|rate|election|president|world cup|championship)\b/.test(
        q,
      );
    const hasReliableCategory = tags.some((t) =>
      ["Crypto", "Politics", "Finance", "Sports"].includes(t),
    );
    checks.quality =
      !isPersonalTrivial &&
      !isTooGeneric &&
      (hasCryptoOrFinance || hasReliableCategory);
    ...
    const verdict =
      score >= 70 ? "approved" : score >= 50 ? "review" : "rejected";
    let reason = "";
    if (!checks.legal) {
      reason = "Market violates content policy and cannot be listed on Arken.";
    } else if (!checks.quality) {
      reason = isPersonalTrivial
        ? "Personal or trivial questions are not suitable for prediction markets."
        : "Market lacks real-world significance. Use Crypto, Finance, Politics, or Sports topics.";
    } else if (!checks.resolvable) {
      reason =
        "Market needs a specific timeframe and verifiable outcome to be resolvable.";
    } else if (!checks.clear) {
      reason =
        "Question structure is unclear. Use at least 5 words and end with a question mark.";
    } else if (verdict === "approved") {
      reason =
        "Market is clear, verifiable, and meets Arken quality standards.";
    } else {
      reason =
        "Market passes basic checks but needs human review before going live.";
    }
  ...
  const handleSubmit = async () => {
    ...
    if (!initialLiquidity || Number(initialLiquidity) < 1) {
      toast.error(
        `Initial liquidity must be at least 1 ${chain === "EVM" ? "USDT" : "USDC"}`,
      );
      return;
    }
    if (!telegramId) {
  ...
  const netLiq = initialLiquidity
    ? Number(initialLiquidity).toFixed(2)
    : "0.00";
  ...
  const canSubmit =
    ...
    endDate.includes("T") &&
    (marketType === "binary" ||
      multiOptions.filter((o) => o.label.trim().length > 0).length >= 2);

  const aiApproved =
    aiState === "done" &&
    aiResult &&
    (aiResult.verdict === "approved" || aiResult.verdict === "review");
  const aiRejected =
    aiState === "done" && aiResult && aiResult.verdict === "rejected";
  ...
  const scoreColor = aiResult
    ? aiResult.score >= 70
      ...
      : aiResult.score >= 50
        ...
        : C.red
    : C.muted;
  const scoreLabel = aiResult
    ? aiResult.verdict === "approved"
      ? "Auto Approved"
      : aiResult.verdict === "review"
        ? "Needs Review"
        : "Rejected"
    : "";
    ...
    >
      ...
      >
        ...
        <div style={card}>
            ...
            {[
              {
                val: "binary",
                title: "YES / NO",
                desc: "Simple binary outcome",
                ex: "Will BTC hit $200K?",
              },
              {
                val: "multi",
                title: "Multi-Option",
                desc: "Multiple outcomes",
                ex: "Who wins the World Cup?",
              },
            ].map((t) => {
              ...
              return (
                <button
                  key={t.val}
                  onClick={() => setMarketType(t.val)}
                  style={{
                    padding: "14px 12px",
                    borderRadius: 14,
                    border: `2px solid ${active ? C.purple : C.border}`,
                    background: active ? C.purpleGlow : C.surface,
                    cursor: "pointer",
                    textAlign: "left",
                  }}
                >
            ...
            <div>
              ...
              >
                ...
                >
                  ...
                  <div
                    style={{ color: C.green, fontWeight: 800, fontSize: 20 }}
                  >
                    {yesProb}%
                  </div>
                </div>
                ...
                >
                  ...
                  <div style={{ color: C.red, fontWeight: 800, fontSize: 20 }}>
                    {100 - yesProb}%
                  </div>
                </div>
              </div>
          ...
          {marketType === "multi" && (
            <div>
              ...
              >
                <div style={{ ...label11, marginBottom: 0 }}>
                  OUTCOMES (MIN 2, MAX 8)
                </div>
                <div
                  style={{
                    fontSize: 11,
                    fontWeight: 700,
                    color: multiTotal === 100 ? C.green : C.red,
                  }}
                >
                  Total: {multiTotal}%{" "}
                  {multiTotal === 100 ? "✓" : "(must = 100%)"}
                </div>
              </div>
                  ...
                  return (
                    <div
                      ...
                      style={{
                        background: C.surface,
                        borderRadius: 12,
                        border: `1.5px solid ${opt.label ? col + "50" : C.border}`,
                        padding: "10px 12px",
                      }}
                    >
                      <div
                      ...
                      >
                        ...
                        <input
                          value={opt.label}
                          onChange={(e) =>
                            updateMultiLabel(opt.id, e.target.value)
                          }
                          placeholder={`Option ${i + 1}${i === 0 ? " e.g. Brazil" : i === 1 ? " e.g. Australia" : ""}`}
                          style={{
                            flex: 1,
                        ...
                        {multiOptions.length > 2 && (
                          <button
                            onClick={() => removeMultiOption(opt.id)}
                            style={{
                              background: `${C.red}15`,
                              color: C.red,
                              border: "none",
                      ...
                      >
                        <input
                          ...
                          value={opt.prob}
                          onChange={(e) =>
                            updateMultiProb(opt.id, e.target.value)
                          }
                          style={{
                            flex: 1,
                            accentColor: col,
              ...
              {multiOptions.length < 8 && (
                <button
                  ...
                  style={{
                    ...
                    color: C.purpleL,
                    border: `1px solid ${C.purpleGlow2}`,
                    borderRadius: 12,
                    padding: "10px 0",
                    fontSize: 13,
                    fontWeight: 600,
              ...
              >
                Set initial odds per outcome. Total must equal 100%. Editing one
                option auto-adjusts others.
              </div>
            </div>
          )}
        </div>
        ...
        <div style={card}>
          ...
          >
            {[
              { val: false, label: "Public", desc: "Anyone can join" },
              { val: true, label: "Private", desc: "Invite code only" },
            ].map((v) => {
              const active = isPrivate === v.val;
              return (
                <button
                  key={v.label}
                  onClick={() => setIsPrivate(v.val)}
                  style={{
                    padding: "14px 10px",
                    borderRadius: 14,
                    border: `2px solid ${active ? C.purple : C.border}`,
          ...
          {isPrivate && (
            ...
            >
              <div style={{ ...label11, marginBottom: 10 }}>
              ...
              <div style={{ color: C.muted, fontSize: 12 }}>
                Only people with this code can join your private market
              </div>
            </div>
          )}
        </div>
        ...
        {initialLiquidity && Number(initialLiquidity) >= 1 && (
          <div style={card}>
            ...
            <p style={{ fontSize: 12, color: C.muted, marginBottom: 10 }}>
              Which outcome are you backing with your{" "}
              {Number(initialLiquidity).toFixed(2)} {currency}?
            </p>
            <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
              {(marketType === "binary"
                ? ["Yes", "No"]
                : multiOptions.map(
                    (o) => o.label || `Option ${multiOptions.indexOf(o) + 1}`,
                  )
              ).map((label, idx) => {
          ...
          {!isPrivate ? (
            <div>
              ...
              >
                ...
                <div>
                  <div
                    style={{ fontWeight: 700, fontSize: 14, color: "#60a5fa" }}
                  >
                    UMA Oracle
                  </div>
                  <div style={{ color: C.muted, fontSize: 12, marginTop: 2 }}>
                    On-chain automatic resolution via Chainlink data feeds
                  </div>
                </div>
              ...
              >
                Public markets require UMA Oracle for trustless, decentralized
                resolution.
              </div>
            </div>
          ) : (
            <div>
              ...
              {oracleType === "manual" && (
                ...
                >
                  You will manually resolve this market. Only available for
                  private markets.
                </div>
              )}
            </div>
        ...
        <div style={card}>
          ...
          >
            Define exactly when this market resolves YES or NO. Be specific —
            this is binding on-chain.
          </div>
          ...
          <textarea
            ...
            rows={3}
            style={{
              ...textareaStyle(!!criteriaYes),
              border: `1.5px solid ${criteriaYes ? C.green : C.border}`,
              marginBottom: 12,
            }}
          />
          ...
          <textarea
            ...
            rows={2}
            style={{
              ...textareaStyle(!!criteriaNo),
              border: `1.5px solid ${criteriaNo ? C.red + "80" : C.border}`,
              marginBottom: 16,
            }}
          />
            ...
            <div style={{ display: "flex", gap: 8 }}>
              <input
                ...
                style={{
                  ...
                  minWidth: 0,
                  background: C.surface,
                  border: `1.5px solid ${endDate ? C.purple : C.border}`,
                  borderRadius: 10,
                  padding: "11px 10px",
                  color: endDate ? C.text : C.muted,
                  fontSize: 14,
                  outline: "none",
                  colorScheme: "dark",
                }}
          ...
          <div>
            ...
            {dataSources.length > 0 && (
              ...
              >
                Primary:{" "}
                <span style={{ color: C.purpleL, fontWeight: 600 }}>
                  {dataSources[0]}
                </span>
        ...
        >
          ...
          >
            ...
            <div>
              ...
              <div style={{ color: C.muted, fontSize: 12 }}>
                Powered by Claude — instant quality check
              </div>
            </div>
          </div>
          ...
          {aiState === "loading" && (
            <div style={{ textAlign: "center", padding: "16px 0" }}>
              <div style={{ color: C.purpleL, fontSize: 13, marginBottom: 8 }}>
              ...
              >
                <div
                  style={{
                    ...
                    height: 64,
                    borderRadius: "50%",
                    background: `conic-gradient(${scoreColor} ${aiResult.score * 3.6}deg, ${C.border} 0deg)`,
                    display: "flex",
                    alignItems: "center",
                    justifyContent: "center",
                    flexShrink: 0,
                ...
                <div>
                  <div
                    style={{ fontWeight: 700, fontSize: 15, color: scoreColor }}
                  >
                    {scoreLabel}
                  </div>
                  <div
                  ...
                  return (
                    <div
                      ...
                      style={{
                        ...
                        alignItems: "center",
                        gap: 8,
                        background: `${pass ? C.green : C.red}0d`,
                        border: `1px solid ${pass ? C.green : C.red}25`,
                        borderRadius: 10,
                        padding: "8px 12px",
                      }}
                    >
                      <span
                        style={{ color: pass ? C.green : C.red, flexShrink: 0 }}
                      >
                        {pass ? <CheckIcon /> : <WarningIcon />}
                      </span>
                      <span
                        style={{
                          fontSize: 12,
        ...
        {isPrivate && oracleType === "manual" && (
          ...
          >
            <div style={{ flexShrink: 0, color: "#f59e0b", marginTop: 1 }}>
              <WarningIcon />
            </div>
            <div>
              <div
              ...
              >
                Manual Resolution Market
              </div>
              <div style={{ fontSize: 12, color: C.muted, lineHeight: 1.6 }}>
                This market will be resolved manually by the creator.
                Participants should trust you before joining.
              </div>
            </div>
          </div>
        )}
        ...
        {!isPrivate && canSubmit && aiState === "idle" && (
          ...
          >
            Public markets require AI review before submission. Run the review
            above first.
          </div>
        )}
        ...
        {!isPrivate && aiRejected && (
          ...
          >
            <div style={{ flexShrink: 0, color: C.red, marginTop: 1 }}>
              <WarningIcon />
            </div>
            <div>
              <div
              ...
              >
                Market Blocked by AI
              </div>
              <div style={{ fontSize: 12, color: C.muted, lineHeight: 1.6 }}>
                This market did not pass quality review and cannot be listed
                publicly. Edit your question and re-analyze.
              </div>
            </div>
          </div>
        )}
        ...
        <button
          onClick={
            loading
              ? undefined
              : !isPrivate && aiState === "idle" && canSubmit
                ? runAI
                : handleSubmit
          }
          disabled={
          ...
          style={{
            ...
            opacity:
              loading || (!isPrivate && !canFinalSubmit && aiState !== "idle")
                ? 0.6
                : 1,
        ...
        >
          {loading
            ? "Deploying..."
            : aiRejected
              ? "Rejected — Edit & Re-analyze"
              : !isPrivate && aiState === "idle" && canSubmit
                ? "Run AI Review First"
                : aiState === "loading"
                  ? "Analyzing..."
                  : isPrivate
                    ? "Submit Private Market"
                    : "Submit Market"}
        </button>
