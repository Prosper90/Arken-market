...
function Dashboard() {
  ...
  const getUserDetails = async (page, Keyword = filterKeyword) => {
    setLoader(true);
    var datas = {
      apiUrl: apiService.get_all_user_fiat_deposit,
      payload: {
        currentPage: page,
        pageSize: pageSize,
  ...
  const download_csv = () => {
    if (csvDataref.current?.length > 0) {
      toast.success("Bank Deposit Download Successfully.");
    } else {
              ...
              <div className="px-4 transaction_padding_top">
                <div className="px-2 my-4 transaction_padding_top tops">
                  <div className="headerss">
                    <span className="dash-head">Bank Deposit History</span>
  ...
  {Usersdataref.current.length > 0 ? (
    Usersdataref.current.map((item, i) => (
      <tr key={item._id}>
        ...
        <td>
          <span className="plus_14_ff">{item?.amount || 0}</span>
        </td>
        ...
        <td>
          <label className="plus_14_ff">
            {item?.receivedAmount ? parseFloat(item.receivedAmount).toFixed(2) : "0.00"}
          </label>
        </td>
       <td>
  <label className="plus_14_ff">
    ...
    <span>
      <i
        className="fa-solid fa-copy copy-key"
  ...
  {item?.status === "CREATED" || item?.status === "PENDING" ? (
    <label className="plus_14_ff text-warning">Pending</label>
  ) : item?.status === "PAID" ? (
    <label className="plus_14_ff text-success">Completed</label>
  ) : (
    <label className="plus_14_ff text-danger">Cancelled</label>
  )}
</td>
